...
use App\Interfaces\RepositoryCriteriaInterface;
...
abstract class BaseRepository implements RepositoryInterface, RepositoryCriteriaInterface
{
    ...
    public function pushCriteria($criteria)
    {
        ...
        if (!$criteria instanceof CriteriaInterface) {
            throw new \Exception("Class " . get_class($criteria)
     ...
     * @return mixed|string
     */
    public function getSearchValue(&$field, &$relation, &$condition, $options)
    {
        if (is_numeric($field)) {
            $field = $condition;
            $condition = "=";
        ...
        $value = array_get($options, $dataKey);
        ...
        if ($value && ($condition == "like" || $condition == "ilike")) {
            $value = "%{$value}%";
        }
    ...
    public function chunkRecord($column, $limit = 1000)
    {
        ...
        if ($column) {
            $this->model->select($column)->chunk($limit, function ($records) use ($collection) {
                foreach ($records as $element) {
                    $collection->push($element);
                }
            });
        } else {
            $this->model->chunk($limit, function ($records) use ($collection) {
                foreach ($records as $element) {
                    $collection->push($element);
                }
            });
